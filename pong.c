...
void game() {
    ...
    while (score_left != 21 || score_right != 21) {
        ...
        switch (input) {  // расчёт движения ракеток
            case 1:
                if (left_rak_pos < 25)
                    left_rak_pos++;  // Используйте максимально возможную позицию для ракетки
                break;
            case 2:
        ...
        if (ball_x == 82 &&
            (ball_y == right_rak_pos || ball_y == right_rak_pos + 1 || ball_y == right_rak_pos - 1)) {
            ball_dir_x = -1;
        } else if (ball_x == 5 &&
                   (ball_y == left_rak_pos || ball_y == left_rak_pos + 1 || ball_y == left_rak_pos - 1)) {
            ball_dir_x = 1;
        }
...
int action() {
    int bazalbek, ch;
    printf(
        "\033[35;0H write action + enter(a,z to move left racket, k,m to move right racket, space for "
        "skip)\n");
    ch = getchar();
    while (getchar() != '\n')
        ;
